...
export default function Home() {
  ...
  const increaseDecreaseDate = (increase = false) => {
    ...
    const value = increase ? lengthOfOneDayInMiliseconds : (-1) * lengthOfOneDayInMiliseconds;
    setDate((prevDate) => {
  ...
  return (
    ...
    >
      ...
      <div className={styles.content}>
        ...
        <CSSTransition in={showContent} timeout={500} nodeRef={contentRef} classNames={{
          enter: `animate__animated animate__bounce${shiftLeft ? 'InRight' : 'InLeft'}`,
          enterDone: `animate__animated animate__bounce${shiftLeft ? 'InRight' : 'InLeft'}`,
          exit: `animate__animated animate__bounce${shiftLeft ? 'OutLeft' : 'OutRight'}`,
        }} onExited={() => setIsContentOut(true)} onEntered={() => setIsContentOut(false)}>
          <div className={styles.imageWithDescription} ref={contentRef}>
            ...
            <div className={styles.image}>
              {currentContent}
              {description && <div className={classnames(styles.description, {[styles.show]: showDescription})}>{description}</div>}
            </div>
          </div>
        </CSSTransition>
